...
const releases = await fetch("https://api.github.com/repos/Nuncord/Nunbop/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "org.nuncord.nunbop.metainfo.xml");
...
await fs.writeFile("./dist/org.nuncord.nunbop.metainfo.xml", output, "utf-8");
console.log("Updated meta information written to ./dist/org.nuncord.nunbop.metainfo.xml");
