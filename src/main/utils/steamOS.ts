...
const layoutId = "3080264545"; // Nunbop Layout v2
...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Nunbop Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Nunbop's recommended Steam Deck controller settings?
